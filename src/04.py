...
for i in range(len(matrix)):
   ...
   if i != 0: to_check.append(np.diag(matrix,k=-i))
for i in range(len(rot_matrix)):
   to_check.append(np.diag(rot_matrix,k=i))
   if i != 0: to_check.append(np.diag(rot_matrix,k=-i))
for row in to_check:
...
def check_for_x_mas(matrix,coord):
   if (coord[0] > 0 and coord[0]< matrix.shape[0]-1 and coord[1] > 0 and coord[1]< matrix.shape[1]-1):
    ...
    top_mas = (top_left == 'M' and bottom_right == 'S') or (top_left == 'S' and bottom_right == 'M')
    bottom_mas =  (bottom_left == 'M' and top_right == 'S') or (bottom_left == 'S' and top_right == 'M')
...
for coord in a_locations:
   if check_for_x_mas(matrix,coord): found_count += 1
